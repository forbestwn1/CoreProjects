...
public class HAPUINode {
	...
	public HAPUIDataInfo getDataInfo(String name) {
		...
		HAPInfoReferenceResolve resolve = HAPUtilityContext.resolveElementReference(new HAPReferenceElement(name), this.getStoryNode().getDataStructureInfo().getContext(), Arrays.asList(HAPStructureValueDefinitionGroup.getVisibleContextTypes()).toArray(new String[0]), HAPUtilityConfiguration.getContextProcessConfigurationForUIUit(HAPConstantShared.UIRESOURCE_TYPE_RESOURCE).relativeResolveMode);

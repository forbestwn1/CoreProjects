		...
		case HAPConstant.CONTEXTSTRUCTURE_TYPE_NOTFLAT:
			...
			for(String rootName : mapping.getElementNames()) {
				...
				if(outputStructure.getElement(rootName)!=null) {
					Map<String, HAPMatchers> matchers = HAPUtilityContext.mergeContextRoot(outputStructure.getElement(rootName), mapping.getElement(rootName), HAPProcessorDataAssociation.getModifyStructure(daProcessConfigure), contextProcessRequirement);

...
public class HAPProcessorContext {
	...
	public static HAPContextStructure process(HAPContextStructure context, HAPParentContext parent, List<HAPServiceData> errors, HAPConfigureContextProcessor configure, HAPRuntimeEnvironment runtimeEnv) {
		HAPContextStructure out = null;
		if(context!=null) {
			String type = context.getType();
			if(type.equals(HAPConstantShared.CONTEXTSTRUCTURE_TYPE_FLAT)) {
				out = process((HAPContext)context, parent, new HashSet<String>(), errors, configure, runtimeEnv);
			}
			else if(type.equals(HAPConstantShared.CONTEXTSTRUCTURE_TYPE_NOTFLAT)) {
				out = process((HAPContextGroup)context, parent, new HashSet<String>(), errors, configure, runtimeEnv);
			}
		}
		return out;
	}
	...
	public static HAPContext process(HAPContext context, HAPParentContext parent, Set<String> dependency, List<HAPServiceData> errors, HAPConfigureContextProcessor configure, HAPRuntimeEnvironment runtimeEnv) {
		...
		HAPContextGroup processed = process(contextGroup, parent, dependency, errors, configure, runtimeEnv);
	...
	public static HAPContextGroup process(HAPContextGroup contextGroup, HAPParentContext parent, Set<String>  dependency, List<HAPServiceData> errors, HAPConfigureContextProcessor configure, HAPRuntimeEnvironment runtimeEnv) {
		if(configure==null)  configure = new HAPConfigureContextProcessor();
		HAPContextGroup out = processStatic(contextGroup, parent, errors, configure, runtimeEnv);
	...
	public static HAPContextGroup processStatic(HAPContextGroup contextGroup, HAPParentContext parent, List<HAPServiceData> errors, HAPConfigureContextProcessor configure, HAPRuntimeEnvironment runtimeEnv) {
		if(configure==null)  configure = new HAPConfigureContextProcessor();

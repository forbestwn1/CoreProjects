...
public class HAPContainerVariableCriteriaInfo extends HAPSerializableImp{
	...
	public boolean addVariableCriteriaInfo(HAPInfoCriteria criteriaInfo, Set<String> aliases) {
		...
		for(String alias : aliases) {
			...
			if(oldVarId!=null) {
				//alias exist already, remov it first
				this.m_varIdByName.remove(aliases);
	...
	public void addVariableCriteriaInfo(HAPInfoCriteria criteriaInfo, String alias) {
		Set<String> aliases = new HashSet<String>();
		aliases.add(alias);
		this.addVariableCriteriaInfo(criteriaInfo, aliases);
	}

	public Set<String> getVariableAlias(String varId){
		return this.m_namesByVarId.get(varId);
	}

	public Set<String> getDataVariableNames(){		return this.m_varIdByName.keySet();	}
	
	public Map<String, HAPInfoCriteria> getVariableCriteriaInfos(){
		return this.m_criteriaInfosById;
	}
	
	public HAPInfoCriteria getVariableCriteriaInfoByAlias(String name) {
		return this.m_criteriaInfosById.get(this.m_varIdByName.get(name));
	}
	...
	public HAPContainerVariableCriteriaInfo groupVariables(Set<String> aliases) {
		...
		for(String varId : group.keySet()) {
			out.addVariableCriteriaInfo(this.m_criteriaInfosById.get(varId), group.get(varId));
		}
		return out;
	}

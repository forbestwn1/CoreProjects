...
public class HAPProcessorContextRule {
	...
	public static HAPContextStructureValueDefinitionGroup process(HAPContextStructureValueDefinitionGroup orgContext, HAPRuntimeEnvironment runtimeEnv) {

		for(String group : orgContext.getAllContextTypes()) {
			HAPContextStructureValueDefinitionFlat context = orgContext.getContext(group);
			for(String eleName : context.getElementNames()) {
				HAPUtilityContext.processContextRootElement(context.getElement(eleName), eleName, new HAPProcessorContextDefinitionElement() {
					@Override
					public Pair<Boolean, HAPElement> process(HAPInfoElement eleInfo, Object obj) {
						if(eleInfo.getContextElement().getType().equals(HAPConstantShared.CONTEXT_ELEMENTTYPE_DATA)) {
							HAPElementLeafData dataEle = (HAPElementLeafData)eleInfo.getContextElement();

...
nosliw.runtime.getResourceService().importResource({"id":{"id":"multioptions",
"type":"uiTag"
},
"children":[],
...
function (env) {
    ...
    var loc_setupUIEvent = function () {
        loc_view.change(function () {
            var request = loc_env.getBatchDataOperationRequest([env.getDataOperationSet(loc_dataVariable, "", loc_getViewData())]);
    ...
    var loc_out = {preInit: function () {
        ...
        var request = loc_env.executeExecuteOperationRequest("test.options;1.0.0", "all", operationParms, {success: function (request, optionsValueArray) {

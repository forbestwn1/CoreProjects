...
var node_createVariableWrapper = function(data1, data2, adapterInfo, requestInfo){
	...
	loc_resourceLifecycleObj[node_CONSTANT.LIFECYCLE_RESOURCE_EVENT_INIT] = function(data1, data2, adapterInfo, requestInfo){
		...
		loc_out.prv_variable.registerDataChangeEventListener(loc_out.prv_dataChangeEventObject, function(event, eventData, request){loc_out.prv_dataChangeEventObject.triggerEvent(event, eventData, request);}, loc_out);
	};	

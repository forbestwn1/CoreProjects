...
var node_createWraperCommon = function(data, path, request){
	...
	loc_resourceLifecycleObj[node_CONSTANT.LIFECYCLE_RESOURCE_EVENT_INIT] = function(obj, path, request){
		...
		if(loc_out.pri_dataBased==false){
			...
			loc_out.pri_parent.registerDataOperationEventListener(this.pri_dataOperationEventObject, function(event, path, opValue, requestInfo){
...
packageObj.createChildNode("createWraperCommon1", node_createWraperCommon); 

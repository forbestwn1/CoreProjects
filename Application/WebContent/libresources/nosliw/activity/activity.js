...
var node_createActivity = function(activityDef, envObj){
	...
	var loc_createContextIO = function(inputValue){
		loc_activityContextIO = node_createIODataSet(loc_processDef[node_COMMONATRIBUTECONSTANT.EXECUTABLEPROCESS_INITSCRIPT](inputValue));
	};
		...
		if(activityCategary==node_COMMONCONSTANT.ACTIVITY_CATEGARY_START){
			...
			activitExecuteRequest = loc_getExecuteNormalActivityRequest(activity, {
				success : function(requestInfo, normalActivityOutput){
					return loc_activityContextIO.getGetDataValueRequest(undefined, {
	...
	var loc_out = {
		...
		getExecuteActivityRequest : function(inputValue, handlers, request){
		...
		executeActivityRequest : function(inputValue, handlers, request){
			var requestInfo = this.getExecuteActivityRequest(inputValue, handlers, request);

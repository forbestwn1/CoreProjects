function(complexEntityDef, valueContextId, bundleCore, configure){
	...
	var loc_out = {
		...
		getEntityInitRequest : function(handlers, request){
			return loc_bundleCore.getDynamicInputContainer().prepareDyanmicInputRequest(loc_tasksInputName, handlers, request);
		},
		...
		getTaskExecuteRequest : function(handlers, request){
			...
			_.each(dynamicInputs, function(dynamicInput){
				out.addRequest(node_taskExecuteUtility.getExecuteInteractiveBrickPackageRequest(dynamicInput.getCoreEntityPackage(), loc_taskCore.getTaskSetup()), {

		...
		if(out!=null) {
			...
			if(REFERENCEID.equals(path)) {
				...
				if(this.m_referenceId!=null) {
					...
					HAPServiceProfile serviceProfile = runtimeEnv.getServiceManager().getServiceProfile(m_referenceId);
...
//					HAPInfoServiceStatic serviceDef = runtimeEnv.getServiceManager().getServiceDefinitionManager().getServiceInfo(this.m_referenceId).getStaticInfo();
					out.addExtendChange(new HAPStoryChangeItemPatch(this.getElementId(), EXTRA, serviceProfile));
					out.addExtendChange(new HAPStoryChangeItemPatch(this.getElementId(), DISPLAYRESOURCE, serviceProfile.getDisplayResource()));
				}

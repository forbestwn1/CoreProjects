...
public abstract class HAPStoryElementImp extends HAPEntityInfoImp implements HAPStoryElement{
	...
	private HAPStoryStatus m_status;
	...
	public HAPStoryElementImp() {
		this.m_status = new HAPStoryStatus();
		this.m_enable = true;
	...
	@Override
	public HAPStoryIdElement getElementId() {	return new HAPStoryIdElement(this.m_categary, this.getId());	}
	...
	@Override
	public HAPStoryStatus getElementStatus() {  return this.m_status;  }
	...
	public HAPStoryChangeResult patch(String path, Object value, HAPRuntimeEnvironment runtimeEnv) {
		...
		HAPStoryChangeResult out = new HAPStoryChangeResult(HAPStoryElement.class);
		if(ENABLE.equals(path)) {
			out.addRevertChange(HAPStoryUtilityChange.buildChangePatch(this, path, this.m_enable));
			this.m_enable = (Boolean)value;
			return out;
		}
		else if(NAME.equals(path)) {
			out.addRevertChange(HAPStoryUtilityChange.buildChangePatch(this, path, this.getName()));
			...
			out.addRevertChange(HAPStoryUtilityChange.buildChangePatch(this, path, this.getDescription()));
			this.setDescription((String)value);
			return out;
		}
		else if(DISPLAYRESOURCE.equals(path)) {
			out.addRevertChange(HAPStoryUtilityChange.buildChangePatch(this, path, this.getDisplayResource()));
			this.setDisplayResource(value);
			return out;
		}
		else if(EXTRA.equals(path)) {
			out.addRevertChange(HAPStoryUtilityChange.buildChangePatch(this, path, this.getExtra()));
			this.setExtra(value);
			return out;
		}
			...
			if(pathSegs[0].equals(HAPEntityInfo.INFO)) {
				...
				out.addRevertChange(HAPStoryUtilityChange.buildChangePatch(this, path, oldValue));
				return out;
			}
		}
		return null;

	...
	protected void parseComplexDefinitionContentJson(HAPManualDefinitionBrick entityDefinition, JSONObject jsonObj, HAPManualDefinitionContextParse parseContext) {
		...
		for(int i=0; i<activityArray.length(); i++) {
			...
			if(HAPUtilityEntityInfo.isEnabled(activityObj)) {
				flowBrick.addActivity((HAPManualDefinitionFlowActivityTask)HAPManualDefinitionUtilityParserBrickFormatJson.parseBrick(activityObj, HAPEnumBrickType.TASK_TASK_ACTIVITYTASK_100, parseContext, getManualDivisionEntityManager(), getBrickManager()));
			}
		}
	}
}
